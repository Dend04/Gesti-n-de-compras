...
export default function FileTable() {
  ...
  const selectedExcel = files.find(
    (f) => selectedIds.includes(f.id) && f.type === "excel",
  );
  const selectedCsv = files.find(
  ...
  const handleCreateTicket = async () => {
    ...
    formData.append(
      "excelFile",
      new Blob([selectedExcel.content], { type: "text/csv" }),
      selectedExcel.name,
    );
    formData.append(
      "csvFile",
      new Blob([selectedCsv.content], { type: "text/csv" }),
      selectedCsv.name,
    );

    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/excel/create-ticket`,
        {
          method: "POST",
          body: formData,
        },
      ...
      const data = await res.json(); // { ticketCsv, sinStockCsv, ticketFileName, sinStockFileName }

      // Crear archivo del ticket
      const ticketFile: ImportedFile = {
        id: `${Date.now()}-ticket-${selectedExcel.name}`,
        name: data.ticketFileName,
        type: "excel creado",
        content: data.ticketCsv,
        importedAt: Date.now(),
      };

      // Crear archivo de productos sin stock
      const sinStockFile: ImportedFile = {
        id: `${Date.now()}-sinstock-${selectedExcel.name}`,
        name: data.sinStockFileName,
        type: "excel creado", // mismo tipo, se distinguirá por el nombre
        content: data.sinStockCsv,
        importedAt: Date.now(),
      };

      addFile(ticketFile);
      addFile(sinStockFile);
      setSelectedIds([]);
    } catch (err: any) {
      alert(err.message);
    }
  ...
  const handleDownloadFormatted = async (file: ImportedFile) => {
    try {
      const isSinStock = file.name.startsWith("SinStock_");
      const endpoint = isSinStock
        ? "/api/excel/download-sinstock"
        : "/api/excel/download-formatted";
